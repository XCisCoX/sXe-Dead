...
struct hook_t
{
	...
	void* Hook;
	unsigned char  Jmp[6];
	unsigned char  APIBytes[6];
	void* APIFunction;
};

namespace hook
{
	bool InitializeHook(hook_t* Hook, wchar_t* ModuleName, char* FunctionName, void* HookFunction)
	{
		HMODULE hModule;
		ULONG OrigFunc, FuncAddr;
		unsigned char opcodes[] = { 0x90, 0x90, 0x90, 0x90, 0x90, 0xe9, 0x00, 0x00, 0x00, 0x00 };
